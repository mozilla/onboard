...
exports.newtabUtils = {
    ...
    dismissSnippet: function() {
        ...
        // moves the tour forward 1 step but, only if the mainCTA
        // has not been completed and, we are not on the final step
        if (!mainCTAComplete && step !== 5) {
            storageManager.set('step', parseInt(step) + 1);
        // if the mainCTA is not complete and we are on the final step
        } else if (!mainCTAComplete && step === 5) {
            // store an indicator that the main tour is complete
            storageManager.set('mainTourComplete', true);
        }

        // store the dismissed snippet in the dismissedSnippets array
        dismissedSnippets.push(step);
        storageManager.set('dismissedSnippets', dismissedSnippets);
    },
    /**
     * Loads and returns the requested snippet
     * @param {int} dismissedStep - The specific previously dismissed step
     * @returns The processed HTML as a string
     */
    getSnippet: function(dismissedStep) {
        let mainCTAComplete = storageManager.get('mainCTAComplete');
        ...
        if (typeof step === 'undefined') {
            ...
            snippet = module.exports.newtabUtils.getSnippetHTML(0);
        // If we are past the first step, and the main tour is not complete
        ...
        // If we are past the first step, the main tour is complete, and a
        // dismissedStep has been passed
        } else if (typeof dismissedStep !== 'undefined') {
            // Load the dismissedStep
            snippet = module.exports.newtabUtils.getSnippetHTML(dismissedStep);
            // set mainCTAComplete to false
            storageManager.set('mainCTAComplete', false);
        }
